...
char *prom_getenv(char *envname) //¥”prom_envp÷–ªÒ»°÷∆∂®≤Œ ˝
{
	char **env = prom_envp;
	int i;
...
{ //≤È’“env ‰≥ˆµƒª∑æ≥±‰¡ø,≤È’“∂‘”¶µƒ±‰¡ø£¨’“≤ªµΩ‘Ú”√ƒ¨»œµƒdfl
...
void __init prom_init_cmdline(void)
{
	...
	for (i = 1; i < prom_argc; i++) { //µ⁄“ª∏ˆŒ™g,º˚œ¬√Ê∫Ø ˝
...
//prom∫Ø ˝ «∫Õ”≤º˛œ‡πÿµƒ£¨◊ˆ“ª–©µ◊≤„µƒ≥ı ºªØ£¨Ω” ‹“˝µº◊∞‘ÿ∫Ø ˝¥´∏¯ƒ⁄∫Àµƒ≤Œ ˝
void __init prom_init(void)//ls1c go here
{
	prom_argc = fw_arg0;
	prom_argv = (char **)fw_arg1; //pmonœ‘ æ:g root=/dev/mtdblock1 console=ttyS2,115200 noinitrd init=/linuxrc rw rootfstype=yaffs2 video=ls1bfb:320x240-16@60 fbcon=rotate:1 consoleblank=1
	prom_envp = (char **)fw_arg2; //pmonœ¬÷¥––env ‰≥ˆµƒƒ⁄»›

	mips_machtype = MACH_LS232;
	system_state = SYSTEM_BOOTING;

	prom_init_cmdline(); //prom_argvƒ⁄»›øΩ±¥µΩarcs_cmdline÷–,πÊ∑∂ªØ
...
#if defined(CONFIG_LS1C_MACH) //ls1c go here,∏¥Œªusbøÿ÷∆∆˜£¨∑¿÷π∆Ù∂Ø ±Œﬁπ À¿ª˙,‘›≤ªø¥
	...
	__raw_writel(__raw_readl(LS1X_MUX_CTRL1) | USBHOST_RSTN, LS1X_MUX_CTRL1);
#else
	/* ÈúÄË¶ÅÂ§ç‰Ωç‰∏ÄÊ¨°USBÊéßÂà∂Âô®Ôºå‰∏îÂ§ç‰ΩçÊó∂Èó¥Ë¶ÅË∂≥Â§üÈïøÔºåÂê¶ÂàôÂêØÂä®Êó∂Ëé´ÂêçÂÖ∂Â¶ôÁöÑÊ≠ªÊú∫ */
...
//	#endif
	...
	/* Âè™Áî®‰∫éls1bÁöÑlcdÊé•Âè£‰º†vgaÊé•Âè£Êó∂‰ΩøÁî®ÔºåÂ¶Ç‰∫ëÁªàÁ´ØÔºå
	 Âõ†‰∏∫‰ΩøÁî®vgaÊó∂ÔºåpllÈÄöËøáËÆ°ÁÆóÈöæ‰ª•ÂæóÂà∞ÂêàÈÄÇÁöÑÂàÜÈ¢ë;ÁªôLS1X_CLK_PLL_FREQÂíåLS1X_CLK_PLL_DIV
	 ÂØÑÂ≠òÂô®‰∏Ä‰∏™Âõ∫ÂÆöÁöÑÂÄº */
	{
	int i;
	...
	if (options) {
		...
		for (i=0; i<strlen(options); i++)
			if (isdigit(*(options+i)))
				break;	/* Êü•ÊâæoptionsÂ≠ó‰∏≤‰∏≠Á¨¨‰∏Ä‰∏™Êï∞Â≠óÊó∂iÁöÑÂÄº */
		if (i < 4) {
...
//				(input_vga->refresh == default_refresh)) {
		...
		if (input_vga->ls1b_pll_freq) {
			...
			/* ËÆ°ÁÆóddrÈ¢ëÁéáÔºåÊõ¥Êñ∞‰∏≤Âè£ÂàÜÈ¢ë */
